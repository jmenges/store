...
export default function ActiveFilterChips({ currencySymbol, activeFilters, className }: Props) {
  /* Hooks */
  const { removeFromQueryParams, removePairFromQueryParams } = useQueryParamUpdater();
        ...
        {activeFilters.length > 0 && (
          ...
          >
            <div className="flex items-center gap-2">
              <div className="relative top-0 text-sm font-medium"> Applied Filters: </div>
              <AnimatePresence>
                {activeFilters.map((activeFilter) => {
                  ...
                  if (filterKey === 'price') {
                    ...
                    if (Array.isArray(filterValue)) {
                      try {
                        const values = filterValue.map((value) => decodeURIComponent(value));
                  ...
                  if (Array.isArray(filterValue)) {
                    return filterValue.map((singleFilterValue) => (
                      <Chip
                        ...
                        onRemoveClick={() => {
                          removePairFromQueryParams(filterKey, singleFilterValue);
                        }}
                      >
                        {singleFilterValue}
                      </Chip>
